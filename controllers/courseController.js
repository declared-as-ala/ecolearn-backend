    ...
    const coursesWithProgress = await Promise.all(
      courses.map(async (course) => {
        ...
        if (userId) {
          ...
          const videoProgressPercent = courseProgress?.videoProgressPercent || 0;
...
exports.getCourseById = async (req, res) => {
  try {
    ...
    if (userId) {
      ...
      progress = {
        videoWatched,
        videoProgressPercent: videoProgress?.videoProgressPercent || 0,
        exercisesStatus,
        gamesStatus,
        totalPoints,
...
// Mark video as watched / Update video progress
exports.watchVideo = async (req, res) => {
  try {
    const { courseId } = req.params;
    const { timeSpent, progressPercent } = req.body;
    const userId = req.user._id;

    console.log(`🎬 [watchVideo] Updating video progress for course ${courseId} - ${progressPercent || 0}%`);
    ...
    const videoProgressPercent = Math.min(100, Math.max(0, progressPercent || 0));
    const isCompleted = videoProgressPercent >= 90; // Consider 90%+ as completed
    ...
    // Only award badges if video is completed (90%+)
    let courseBadges = [];
    if (isCompleted) {
      courseBadges = await maybeAwardCourseCompletionBadge(userId, course);
    }
      ...
      while (retries > 0) {
        try {
          ...
          if (error.name === 'VersionError' && retries > 1) {
            console.warn(`⚠️ [submitExercise] VersionError (Optimistic Concurrency), retrying... (${retries} left)`);
            retries--;
    ...
    if (isCompleted && !wasAlreadyCompleted) {
      ...
      while (retries > 0) {
        try {
          ...
          if (error.name === 'VersionError' && retries > 1) {
            console.warn(`⚠️ [submitGame] VersionError (Optimistic Concurrency), retrying... (${retries} left)`);
            retries--;
            await new Promise(resolve => setTimeout(resolve, 200)); // Small backoff
          } else {
            throw error;
          }
        }
